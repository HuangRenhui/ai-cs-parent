...
> **注意**：消息队列（RabbitMQ/Kafka）为目标架构，**当前未接入**。工单通知直接通过 WebSocket 推送，无需经过 MQ。
...
graph TB
    ...
    E -->|返回向量| D
    D -->|向量检索| F[(Milvus / Chroma)]
    F -->|Top-K 相似结果| D
    D -->|相似度过滤| D
    D -->|Rerank重排 ⚠️Chroma路径| D
    D -->|返回 FAQ| C
    C -->|响应| B
    B -->|展示结果| A
```

> **两条路径**：FAQ 走 **Milvus + Dashscope**（主路径）；文档 RAG 走 **Chroma + Ollama + BGE-Reranker**（可选路径）。详见 [知识库手册](知识库手册.md)。
