...
本文档约定 Java 与前端的命名、注释、异常、日志与提交规范。
...
### 4. 语言规范
- **代码注释统一使用中文**（与本仓库现行代码一致）
- 注释风格参照 `ai-cs-knowledge` 模块 `AudioController`：类/方法 Javadoc + 关键逻辑行内注释
- 实体 / DTO 字段在字段上一行加 `/** 字段含义 */`
- 修改代码时同步更新受影响的注释，保持注释与逻辑一致
- 保持注释简洁明了
...
- `BusinessException` / 参数校验 → HTTP **400** + `Result.fail`
- `ModelCallException` → HTTP **503**
- 未捕获异常 → HTTP **500**（不要再返回 HTTP 200 包业务失败）
- 前端 `request.js` 对 `code !== 200` 一律视为失败

用户文本拼进提示词时用 `PromptConst.fill`，禁止 `String.format`。
