...
模型注册可配：`timeoutMs` 单次超时、`maxRetries` 同模型重试(0-5)、`failThreshold` 熔断阈值、`dailyTokenLimit`/`dailyCostLimit` 每日配额（超限顺延免费候选）、`costPer1kIn/Out` 单价（用于成本快照）。

### 8.3 模型用量统计

**Controller**: `AiModelConfigController` | **路径前缀**: `/system/ai-model`

逐次调用由 `ai-cs-job` 从 Redis 流消费落 `cs_model_usage`（token、耗时、成本快照、成败）。

```http
GET /system/ai-model/usage/page?page=1&size=20&modelType=LLM&modelId=&success=
GET /system/ai-model/usage/summary?modelType=LLM&days=7
GET /system/ai-model/usage/recent-fail?minutes=5
```

- `usage/summary` 返回 `totalCalls/successCalls/failCalls/totalTokens/avgLatencyMs/totalCost`。
- `usage/recent-fail` 返回近 N 分钟失败数，供运维台模型告警轮询。

### 8.4 意图配置管理（可配置意图）

**Controller**: `IntentConfigController` | **路径前缀**: `/system/intent`

租户级可配置意图，替代写死枚举。字段：`tenantCode`、`intentCode`、`intentName`、`priority`、`enabled`、`delFlag` 等。

```http
GET    /system/intent/list?tenantCode=default   # 租户启用意图列表（按 priority 升序）
GET    /system/intent/{id}                      # 意图详情
POST   /system/intent/save                      # 保存/更新意图
DELETE /system/intent/delete/{id}               # 逻辑删除意图（del_flag=1）
```

### 8.5 填槽配置管理（多轮填槽）

**Controller**: `SlotFillingController` | **路径前缀**: `/system/slot`

按意图配置槽位，缺槽时追问。

```http
GET    /system/slot/list?tenantCode=default&intentCode=LOGISTICS   # 意图关联槽位列表
GET    /system/slot/{id}                      # 槽位详情
POST   /system/slot/save                      # 保存/更新槽位
DELETE /system/slot/delete/{id}               # 删除槽位
```

### 8.6 数据保留策略（出域与保留）

**Controller**: `DataRetentionController` | **路径前缀**: `/system/data-retention`

字段：`retentionDays`（保留天数，默认 90）、`allowExternalDomain`（是否允许出域，默认 0）、`allowUserDelete`（是否允许用户删除，默认 1）。

```http
GET    /system/data-retention/list?tenantCode=default              # 策略列表
GET    /system/data-retention/get?tenantCode=default&dataType=session   # 指定类型策略
POST   /system/data-retention/save                                 # 保存/更新策略
DELETE /system/data-retention/delete/{id}                          # 删除策略
```
...
```http
POST /knowledge/feedback/{id}?type=like
```

- `type`：`like`（点赞）或 `dislike`（点踩）
- 返回 `Result<String>`，成功后对应计数 +1

### 13.14 未命中转问学习

```http
POST /knowledge/miss/{id}/convert?answer=...&category=账户管理
```

- 将未命中记录（`status=0`）转为正式 FAQ，`audit_status=2`（已发布）并自动向量化
- `answer` 必填；`category` 可选

### 13.15 对外帮助中心

**Controller**: `HelpCenterController` | **路径前缀**: `/help-center`（网关 JWT 白名单，匿名访问）

#### 13.15.1 帮助中心语义检索

```http
GET /help-center/search?question=如何退款&tenantCode=default
```

**响应**：`RagSearchResultDTO`（命中状态、答复、引用）

#### 13.15.2 已发布 FAQ 列表

```http
GET /help-center/faqs?tenantCode=default&category=账户管理&keyword=密码&page=1&size=10
```

**响应**：`PageResult<KnowledgeFaq>`，仅返回「已发布且启用」FAQ，按点赞与浏览热度排序

#### 13.15.3 已发布 FAQ 分类列表

```http
GET /help-center/categories?tenantCode=default
```

**响应**：`List<String>` 分类名列表

#### 13.15.4 帮助中心点赞/点踩

```http
POST /help-center/feedback/{id}?type=like
```

#### 13.15.5 帮助中心浏览量 +1

```http
POST /help-center/view/{id}
```
...
**响应**：`WidgetInitVO`（含 C 端短时访客 JWT，`typ=visitor`，约 2 小时。只能调聊天与会话写入，不能访问工单/客户/运维）。另按 `scene` 从 `cs_scene_config` 解析返回 `greeting`（`{entityId}` 已替换）与 `quickActions`（`[{label, send}]`，Widget 首屏渲染为快捷按钮）。

### 32.1.1 入口场景配置

**Controller**: `OpenSceneConfigController` | **路径前缀**: `/open/scene-config`

```http
GET    /open/scene-config/list
POST   /open/scene-config/save
PUT    /open/scene-config/update
PUT    /open/scene-config/{id}/enabled?enabled=1
DELETE /open/scene-config/delete/{id}
```

内置场景：`ORDER`（订单入口）、`PRODUCT`（产品入口）、`AFTER_SALE`（售后入口）、`GENERAL`（兜底）。场景是提示而非锁：首轮后的对话仍以意图识别为准，退款等写操作仍走确认卡与幂等键。
...
| 系统配置 | 4 | base-service |
| 操作日志 | 1 | base-service |
| 意图配置管理 | 4 | base-service |
| 填槽配置管理 | 4 | base-service |
| 数据保留策略 | 4 | base-service |
| 数据统计 | 5 | base-service |
