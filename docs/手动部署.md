...
裸机部署。端口与启动顺序见 [快速开始](快速开始.md)（8080–8088）。本地开发不要使用下文 `/opt` 路径。网关路由以 `ai-cs-gateway` 的 `application.yml` 为准。
...
```bash
mysql -u ai_cs_user -p < docs/database/init.sql
```

脚本会创建并使用库 `ai_customer_service`（不要再用 `ai_cs`）。
...
创建配置时请复制仓库里各服务的 `application.yml`。网关路由摘要见 [接口说明](接口说明.md)。
...
# JVM 参数
# 按机器内存调整，开发机默认不是生产合同值（见配置说明「服务治理与性能」）
...
SERVICES=(
    ...
    "http://localhost:8084/actuator/health:Base Service"
    "http://localhost:8082/actuator/health:AI Agent"
    "http://localhost:8083/actuator/health:Knowledge"
    "http://localhost:8085/actuator/health:WorkOrder"
    "http://localhost:8081/actuator/health:WebSocket"
    "http://localhost:8086/actuator/health:Open"
    "http://localhost:8087/ops/health/self:Ops"
    "http://localhost:8088/actuator/health:Job"
)
...
# 生产按压测调优，开发机默认不是合同值（见 配置说明「服务治理与性能」）
```
